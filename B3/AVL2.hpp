...
private:
	...
	public:
		...
		Node(T val, Node* parent = nullptr) : data(val), left(nullptr), right(nullptr), parent(parent), balance(0) {}
	};
...
void AVL<T>::balanceRecurcive(Node* current) {
	...
	if (current->parent->right == current) { // From the right subtree.
		if (current->parent->balance > 0) { // Right is heavy.
			if (current->balance < 0) { // From the right-left subtree.
				rotate(current, true);
				rotate(current->parent->parent, false);
			}
			else { // From the right-right subtree.
				rotate(current->parent, false);
			}
		}
		else {
			if (current->parent->balance < 0) { // Becomes balanced.
				current->parent->balance = 0;
				return;
			}
			current->parent->balance += 1; // Becomes right heavy.
			balanceRecurcive(current->parent);
		}
	}
	else { // From the left subtree.
		if (current->parent->balance < 0) { // Left is heavy.
			if (current->balance > 0) { // From the left right subtree.
				rotate(current, false);
				rotate(current->parent->parent, true);
			}
			else { // From the left-left subtree.
				rotate(current->parent, true);
			}
		}
		else {
			if (current->parent->balance > 0) { // Becomes balanced.
			...
			current->parent->balance -= 1; // Becomes left heavy.
			balanceRecurcive(current->parent);
		}
	}
}
